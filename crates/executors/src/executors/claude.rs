...
impl StandardCodingAgentExecutor for Claude {
    async fn spawn(
        ...
        let claude_command = self.command.clone();
    ...
    async fn spawn_follow_up(
        ...
        let claude_command = format!(
            "npx -y @anthropic-ai/claude-code@latest continue {} --output-format=stream-json",
            session_id
        );
...
impl Claude {
    ...
    /// Create a new Claude executor with custom settings
...
impl ClaudeLogProcessor {
    ...
    fn process_logs(_executor: &Claude, msg_store: Arc<MsgStore>, current_dir: &PathBuf) {
...
mod tests {
    ...
    #[test]
    fn test_spawn_follow_up_command_generation() {
        let executor = Claude::new();

        // Test that the follow-up command is correctly formatted
        // We can't easily test the actual spawn without mocking, but we can verify
        // the new() constructor creates the right base command
        assert!(executor.command.contains("--output-format=stream-json"));

        // The follow-up functionality would use:
        // "npx -y @anthropic-ai/claude-code@latest continue {session_id} --output-format=stream-json"
        // This is tested implicitly through the spawn_follow_up method existing
    }
}
