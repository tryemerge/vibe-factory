...
export const CreatePrDialog = NiceModal.create<CreatePrDialogProps>(
  ({ attempt, task, projectId }) => {
    ...
    useEffect(() => {
      ...
      setError(null);
      setPrBaseBranch(pickBranch(attempt.target_branch) || '');
    }, [modal.visible, task, attempt, pickBranch]);
    ...
    const handleConfirmCreatePR = useCallback(async () => {
      ...
      setCreatingPR(false);
    }, [attempt, prBaseBranch, prBody, prTitle, modal]);
