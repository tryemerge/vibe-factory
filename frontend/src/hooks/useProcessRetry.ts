...
function isCodingAgentActionType(
    ...
    return t.type === 'CodingAgentInitialRequest' || t.type === 'CodingAgentFollowUpRequest';
}
...
export function useProcessRetry(attempt: TaskAttempt | undefined) {
    ...
    const { attemptData, refetch: refetchAttempt } = useAttemptExecution(attemptId);
    const { data: branchStatus, refetch: refetchBranch } = useBranchStatus(attemptId);
    ...
    const retryProcess = useCallback(
        async (executionProcessId: string, newPrompt: string) => {
            ...
            const before = (proc as WithBefore | undefined)?.before_head_commit || null;
            ...
            const laterCoding = later.filter((p) => isCodingAgent(p.run_reason)).length;
            const laterSetup = later.filter((p) => p.run_reason === PROCESS_RUN_REASONS.SETUP_SCRIPT).length;
            const laterCleanup = later.filter((p) => p.run_reason === PROCESS_RUN_REASONS.CLEANUP_SCRIPT).length;
            ...
            try {
                modalResult = await showModal<typeof modalResult extends infer T ? T extends object ? T : never : never>(
                    'restore-logs',
                    {
                        targetSha: before,
                        targetSubject,
