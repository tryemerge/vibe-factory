...
<Accordion title="Factory Droid" icon="code">
**Factory AI's Droid coding agent**
...
- **Autonomy Level**: Controls permission level for file and system operations
  ...
  - `medium`: Development operations with recoverable side effects (default)
  - `high`: Production operations with security implications or major side effects
  - `skip-permissions-unsafe`: Bypasses all permission checks (use only in isolated environments)
